...
const WBTCAdmin = artifacts.require('./WBTCManager.sol')

const groupWBTCProxy = artifacts.require('./HTLCWBTC.sol')
...
contract('StoremanAdminSC', ([owner, admin, proxy, storemanGroup])=> {
    ...
    let WBTCInstance;
    let WBTCAdminInstance;
    let coinAdminInst;

    let smgAdminInstance;
    let groupWBTCProxyInst;
    ...
    before('set up contract before test', async () => {
        ...
        groupWBTCProxyInst = await groupWBTCProxy.new({from: owner})


        WBTCAdminInstance = await WBTCAdmin.new(groupWBTCProxyInst.address, smgAdminInstance.address, {from: owner});

        let WBTCInstanceAddr = await WBTCAdminInstance.WBTCToken();
        WBTCInstance = wBTCAbi.at(WBTCInstanceAddr);

    })

    it('initialize contracts', async () => {

        let ratio = 200000; //1 BTC:20,it need to mul the precise 10000
        ...
        let wanchainHtlcAddr = groupWBTCProxyInst.address;
        let wanchainTokenAddr = await WBTCAdminInstance.WBTCToken();
        let wanchainTokenAdminAddr = WBTCAdminInstance.address;
        ...
        await coinAdminInst.setCoinPunishReciever(BTC_ID,account3,{from: owner});

        console.log("set ratio");
        await coinAdminInst.setWToken2WanRatio(BTC_ID, ratio, BTC_EXP,{from: owner});

        exp = await coinAdminInst.mapCoinExponent(BTC_ID)

        console.log("exponent=");
        console.log(exp.toString(10));

        console.log("set delay time");
        await coinAdminInst.setWithdrawDepositDelayTime(BTC_ID, delayTime, {from: owner});
        ...
        await WBTCAdminInstance.setHalt(false, {from: owner});

        console.log("initialize group weth to set token admin");

        await groupWBTCProxyInst.setWBTCManager(wanchainTokenAdminAddr, {from: owner});
        getTokenAdmin = await groupWBTCProxyInst.wbtcManager();
        assert.equal(getTokenAdmin, wanchainTokenAdminAddr, 'wanchainTokenAdminAddr not match');

        await groupWBTCProxyInst.setAdmin(smgAdminInstance.address, coinAdminInst.address, {from: owner});
        smgAdminAddr = await groupWBTCProxyInst.storemanGroupAdmin();
        assert.equal(smgAdminAddr, smgAdminInstance.address, 'wanchainTokenAdminAddr not match');

        await groupWBTCProxyInst.setHalt(false,{from: owner});
      ...
      it('storemanGroupRegisterByDelegate  - [smgAmin-T00900]', async () => {
          ...
          await coinAdminInst.setSystemEnableBonus(BTC_ID,true,10,{from: owner})
          await coinAdminInst.setHalt(false,{from: owner});

          console.log("storemanGroupRegister");
          let regDeposit = web3.toWei(100);

          await  smgAdminInstance.depositSmgBonus(BTC_ID,{from:owner,value:10*regDeposit,gas:4000000});

         let res = await  smgAdminInstance.storemanGroupRegisterByDelegate(BTC_ID,storeManWanAddr1,storeManBTCAddr1,storeManTxFeeRatio,{from:account1,value:regDeposit,gas:4000000});

          periodBlks = 10;
          start = res.receipt.blockNumber;

          getCoinSmgInfo = await smgAdminInstance.mapCoinSmgInfo(BTC_ID,storeManWanAddr1);
          ...
          assert.equal(getOriginalChainAddr,storeManBTCAddr1, 'storeManBTCAddr not match');
          assert.equal(getUnregisterApplyTime,0, 'apply time not match');
          assert.equal(initiator,account1,'initiator is not match');

          console.log("WBTCAdmin getStoremanGroup");
          BTCAdminInfo = await WBTCAdminInstance.getStoremanGroup(storeManWanAddr1);

          console.log(BTCAdminInfo);

          getQuota = new BigNumber(BTCAdminInfo[0].toString(10));

          assert.equal(getQuota.div(BTC_EXP),web3.fromWei(regDeposit/200000*10000), 'quota not match');
  ...
  //  /*
               it('smgClaimSystemBonus By delegate - [smgAmin-T00901]',async () => {
                   ...
                   res = await smgAdminInstance.smgClaimSystemBonusByDelegate(BTC_ID,storeManWanAddr1,{from:account1,gas:1000000});
                   console.log(res);
                   ...
                   start = res.receipt.blockNumber;
                       ...
                       it('StoremanGroupApplyUnregister By delegate - [smgAmin-T00902]',async () => {
                           ...
                           res = await smgAdminInstance.smgApplyUnregisterByDelegate(BTC_ID,storeManWanAddr1,{from:account1,gas:1000000});
                           end = res.receipt.blockNumber;
                           ...
                           coinSmgInfo = await smgAdminInstance.mapCoinSmgInfo(BTC_ID,storeManWanAddr1);
...
///*
                       it('StoremanGroupWithdrawDeposit by delegate  - [smgAmin-T00903]', async () => {
                           ...
                           res = await smgAdminInstance.smgWithdrawDepositByDelegate(BTC_ID,storeManWanAddr1,{from:account1, gas: 4000000})
                           //console.log(res);

                           quotaSet = await WBTCAdminInstance.mapStoremanGroup(storeManWanAddr1);
                           ...
                           assert.equal(web3.fromWei(totalQuota), 0, 'wBTC balance did not set properly');
                           ...
                           gotTxFeeRatio = await smgAdminInstance.getStoremanTxFeeRatio(BTC_ID, storeManWanAddr1);
                       ...
                       it('storemanGroupRegisterByDelegate  - [smgAmin-T00904]', async () => {
                          ...
                          let res = await  smgAdminInstance.storemanGroupRegisterByDelegate(BTC_ID,storeManWanAddr2,storeManBTCAddr2,storeManTxFeeRatio,{from:account1,value:regDeposit,gas:4000000});
                          console.log(res);

                          start = res.receipt.blockNumber;

                           await  smgAdminInstance.depositSmgBonus(BTC_ID,{from:owner,value:regDeposit,gas:4000000});

                           getCoinSmgInfo = await smgAdminInstance.mapCoinSmgInfo(BTC_ID,storeManWanAddr2);
                           console.log(getCoinSmgInfo);
                           ...
                           console.log("WBTCAdmin getStoremanGroup");
                           BTCAdminInfo = await WBTCAdminInstance.getStoremanGroup(storeManWanAddr2);
                       ...
                       it('smgClaimSystemBonus to initiator - [smgAmin-T00905]',async () => {
                           ...
                           res = await smgAdminInstance.storemanGroupClaimSystemBonus(BTC_ID,{from:storeManWanAddr2,gas:1000000});
                           console.log(res);
                       ...
                       it('StoremanGroupApplyUnregister - [smgAmin-T00906]',async () => {
                           ...
                           res = await smgAdminInstance.storemanGroupApplyUnregister(BTC_ID,{from:storeManWanAddr2,gas:1000000});
                           ...
                           coinSmgInfo = await smgAdminInstance.mapCoinSmgInfo(BTC_ID,storeManWanAddr2);
                       ...
                       it('StoremanGroupWithdrawDeposit  - [smgAmin-T00907]', async () => {
                           ...
                           res = await smgAdminInstance.storemanGroupWithdrawDeposit(BTC_ID,{from:storeManWanAddr2, gas: 4000000})
                           //console.log(res);

                           quotaSet = await WBTCAdminInstance.mapStoremanGroup(storeManWanAddr2);
                           ...
                           gotTxFeeRatio = await smgAdminInstance.getStoremanTxFeeRatio(BTC_ID, storeManWanAddr2);
                       ...
                       it('Set punish deposit reciever  - [smgAmin-T00907]', async () => {
                           await coinAdminInst.setHalt(true,{from: owner});
                           let res = await coinAdminInst.setCoinPunishReciever(BTC_ID,account2,{from:owner})
                           await coinAdminInst.setHalt(false,{from: owner});

                           let reciever = await coinAdminInst.mapCoinPunishReceiver(BTC_ID)
                       ...
                       it('test punish deposit transfer  - [smgAmin-T00908]', async () => {
                           ...
                           await coinAdminInst.setHalt(true,{from: owner});
                           console.log("set delay time");
                           await coinAdminInst.setWithdrawDepositDelayTime(BTC_ID, 1, {from: owner});
                           ...
                           res = await smgAdminInstance.punishStoremanGroup(BTC_ID,storeManWanAddr2,100,{from:owner});
                           console.log(res);

                           console.log("================================unregister smg====================================================");
                           initiatorPreBal = web3.fromWei(web3.eth.getBalance(account1));
                           console.log("begin unregister")
                           res = await smgAdminInstance.smgApplyUnregisterByDelegate(BTC_ID,storeManWanAddr2,{from:account1,gas:1000000});
                           console.log(res)
                       ...
                       it('test punish deposit transfer  - [smgAmin-T00909]', async () => {
                          ...
                          // console.log(res)
                           ...
                           res = await smgAdminInstance.punishStoremanGroup(BTC_ID,storeManWanAddr2,50,{from:owner});
                          // console.log(res);
                          ...
                          // console.log(res)
                           ...
                           assert.equal(afterBal-initiatorPreBal>(50 - 2),true,"the transfered deposit should be around 50 WAN")
                           punishRecieverAfterBal = web3.fromWei(web3.eth.getBalance(account2));
                       ...
                       it('test transfer deposit by owner - [smgAmin-T00910]', async () => {
                           ...
                           await  smgAdminInstance.depositSmgBonus(BTC_ID,{from:owner,value:2*regDeposit,gas:4000000});


                           let res = await smgAdminInstance.storemanGroupRegisterByDelegate(BTC_ID, storeManWanAddr2, storeManBTCAddr2, storeManTxFeeRatio, {
                               from: account1,
                               value: regDeposit,
                               gas: 4000000
                           });
                           ...
                           res = await  smgAdminInstance.transferSmgDeposit(BTC_ID,storeManWanAddr2,storeManWanAddr3,false,{from:owner});
                           console.log(res);
                           ...
                           let result = await WBTCAdminInstance.mapStoremanGroup(storeManWanAddr2);
                           console.log(result);
