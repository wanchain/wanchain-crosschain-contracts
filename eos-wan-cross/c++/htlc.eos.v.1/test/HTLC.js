...
async function startAutoTest() {
  ...
  describe("EOS-WAN-Contract-AutoTest", function() {
    ...
    before(async () => {
      ...
      actionTypeListDict = {
        //        user     account   quantity  xHash
        outlock: ['string','string', 'string', 'string'],
        //         npk       account   quantity  xHash
        lockdebt: ['string', 'string', 'string', 'string'],
        //         timeStamp receiver account   sym
        withdraw: {
          //    timeStamp receiver  account   sym
          all: ['string', 'string', 'string', 'string'],
          //      timeStamp receiver  account
          noSym: ['string', 'string', 'string'],
          //          timeStamp receiver
          noAccount: ['string', 'string'],
        }
      };

      buildMpcSign = (sk, typeList, ...args) => {
        let result = {
          R: schnorr.getR(),
          s: schnorr.getS(sk, typeList, args)
        };
    ...
    //     let {R, s} = buildMpcSign(config.skSmgs[pkId], actionTypeListDict.outlock, user1.name, config.sysTokenContract.name, value, xHash);
    ...
    //     let {R, s} = buildMpcSign(config.skSmgs[pkId], actionTypeListDict.outlock, user1.name, tokenAccount, value, xHash);
    ...
    it("cross EOS chain: HTLC storeman1 withdraw, it should be success", async () => {
      try {
        ...
        try {
          ...
          let sym = "4,EOS";
          let {R, s} = buildMpcSign(config.skSmgs[pkId], actionTypeListDict.withdraw.all, nowTime, storeman1.name, config.sysTokenContract.name, sym);
        ...
        try {
          ...
          let {R, s} = buildMpcSign(config.skSmgs[pkId], actionTypeListDict.withdraw.all, nowTime, storeman1.name, config.sysTokenContract.name, invalidSym);
        ...
        try {
          ...
          let nowTime = (Date.now() / 1000).toFixed(0);
          let {R, s} = buildMpcSign(config.skSmgs[pkId], actionTypeListDict.withdraw.all, nowTime, storeman1.name, config.sysTokenContract.name, undefinedSym);
        ...
        let R, s;
        if (tokenAccount.length && tokenSym.length) {
          let mpcSignResult = buildMpcSign(config.skSmgs[pkId], actionTypeListDict.withdraw.all, nowTime, storeman1.name, tokenAccount, tokenSym);
          R = mpcSignResult.R;
          s = mpcSignResult.s;
        } else if (tokenAccount.length) {
          let mpcSignResult = buildMpcSign(config.skSmgs[pkId], actionTypeListDict.withdraw.noSym, nowTime, storeman1.name, tokenAccount);
          R = mpcSignResult.R;
          s = mpcSignResult.s;
        } else {
          let mpcSignResult = buildMpcSign(config.skSmgs[pkId], actionTypeListDict.withdraw.noAccount, nowTime, storeman1.name);
          R = mpcSignResult.R;
          s = mpcSignResult.s;
        }
        log.debug("R:", R);
