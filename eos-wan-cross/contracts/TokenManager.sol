...
contract TokenManager is Halt {
    ...
    /// a map from origin chain token account to corresponding penalty receiver addresses 
    ...
    struct TokenInfo {
        ...
        uint               hashAlgorithms;      /// hash algorithms to caculate xHash
        bytes              htlcOrigAccount;     /// token account on original chain
    }
    ...
    /// @param origHtlc              htlc contract instance account of orginal blockchain chain
    ...
    /// @param storemanGroupAdminAddr  storeman group admin address
    /// @param quotaLedgerAddr       quotaLedger contract instance address
    /// @param wanHtlcAddr           htlc address of wanchain
    function injectDependencies(address storemanGroupAdminAddr, address quotaLedgerAddr, address wanHtlcAddr)
        public
        onlyOwner
        isHalted
    {
        require(storemanGroupAdminAddr != address(0) && quotaLedgerAddr != address(0) && wanHtlcAddr != address(0));
        ...
        onlyOwner
    {
        ...
        candidateInfo.hashAlgorithms = 0;
    }
